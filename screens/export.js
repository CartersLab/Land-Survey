const ExportScreen = (() => {
  ...
  async function _tabReport(obsCount) {
    ...
    const obscureLayerOpts = Object.entries(CONFIG.TILE_PROVIDERS)
      .map(([k, p]) => `<option value="${k}"${(S.obscureBaseLayer||'cartoVoyager')===k?' selected':''}>${p.name}</option>`)
      .join('');
        ...
        <div class="export-section-body">
          ...
          <div class="settings-row" style="padding:8px 0;border-bottom:1px solid var(--border)">
            <div class="settings-row-label">
              ...
              <p style="margin:2px 0 0;font-size:.78rem;color:var(--text-muted)">Satellite imagery, no labels — only loads tiles near observations</p>
            </div>
